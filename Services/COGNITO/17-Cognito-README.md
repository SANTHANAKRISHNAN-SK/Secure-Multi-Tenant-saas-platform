...
| Native API Gateway integration | Amazon API Gateway can attach a Cognito User Pool authorizer directly, validating tokens at API Gateway before any request reaches the ALB/ECS tier |
...
| OAuth Scopes | `openid`, `email`, `profile`, `aws.cognito.signin.user.admin`, plus the custom scopes `saas-api/read` and `saas-api/write` from the Resource Server (below) |
...
| Custom Scope — `read` | `saas-api/read` — API read permission |
| Custom Scope — `write` | `saas-api/write` — API write permission |
| Consumed By | API Gateway Cognito User Pool Authorizer `cognito-authorizer-06`, which validates/authorizes the JWT and enforces the required scope per protected method |

Cognito issues the access token carrying these scopes; **the Resource Server itself does not validate JWTs, and CloudFront does not validate JWTs either** — API Gateway's Cognito User Pool Authorizer is what validates and authorizes the JWT against this Resource Server's scopes.
...
sequenceDiagram
    ...
    APP-->>U: Authenticated session established (access token set as cookie)
    U->>CFFN: Subsequent API calls (Cognito access-token cookie)
    CFFN->>CFFN: Convert cookie to Authorization: Bearer header (token forwarding only)
    CFFN->>AG: Forward request with Authorization header
    AG->>AG: Validate signature, issuer, expiry against JWKS, and required saas-api/* scope (cognito-authorizer-06)
```

Once a token is issued, every subsequent call to a protected route (`/api/v1/users/*`, `/api/v1/admin/*`, `/api/v1/billing/*`) carries the Access Token — forwarded from the browser cookie to an `Authorization: Bearer` header by the CloudFront Function (token forwarding only, no validation) — which the API Gateway Cognito User Pool Authorizer (`cognito-authorizer-06`) validates against Cognito's published JWKS and the Resource Server's `saas-api/read` / `saas-api/write` scopes before forwarding the request to the ALB → ECS tier.
...
| Service | Interaction |
|---|---|
| Amazon API Gateway | Uses the Cognito User Pool Authorizer (`cognito-authorizer-06`) to validate JWTs (signature, issuer, expiry) against the JWKS endpoint and enforce the Resource Server's `saas-api/read` / `saas-api/write` scopes before proxying to the ALB |
| Amazon ECS (Flask App) | Exchanges the authorization code for tokens via the Authorization Code Grant flow (`/auth/callback`) and reads Cognito configuration from environment variables (`COGNITO_APP_CLIENT_ID`, `COGNITO_DOMAIN`, `COGNITO_LOGOUT_REDIRECT_URI`, `COGNITO_REDIRECT_URI`, `COGNITO_REGION`, `COGNITO_USER_POOL_ID`); Flask does not independently validate the JWT as the primary authentication mechanism |
| Amazon CloudFront | The Hosted UI's callback, redirect, and sign-out URLs all point back to the CloudFront distribution domain fronting the application. The CloudFront Function (`cognito-authorizer-cookie-to-header`) forwards the access token from cookie to `Authorization` header only — it does not validate the JWT |
...
- **JWT validation and scope enforcement is centralized at API Gateway** (Cognito User Pool Authorizer `cognito-authorizer-06`), so unauthenticated, tampered, or under-scoped requests never reach the ALB/ECS application tier.
...
- **Scoped OAuth scopes** (`openid`, `email`, `profile`, `aws.cognito.signin.user.admin`, plus the Resource Server custom scopes `saas-api/read` and `saas-api/write`) limit what the issued tokens can be used for.
