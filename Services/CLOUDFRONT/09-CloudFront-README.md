...
## ⚡ CloudFront Function — Token Forwarding

| Attribute | Value |
|---|---|
| Function Name | `cognito-authorizer-cookie-to-header` |
| Trigger | Viewer Request |
| Purpose | Forward the Cognito access token from the browser cookie to API Gateway as an `Authorization: Bearer <Cognito access token>` header |

**The CloudFront Function does NOT validate the JWT.** It only converts the browser cookie into the `Authorization` header required by API Gateway. Signature, issuer, expiry, and scope validation happen downstream at the API Gateway Cognito User Pool Authorizer (`cognito-authorizer-06`) — see [10-APIGateway-README.md](10-APIGateway-README.md).
...
Browser
  → CloudFront
  → CloudFront Function (cognito-authorizer-cookie-to-header)
...
flowchart LR
    ...
    CFFN -->|"Authorization: Bearer <token>"| APIGW["Amazon API Gateway\nrest-api-new-17"]
    APIGW --> ALB["Application Load Balancer"]
    ALB --> ECS["Amazon ECS (Flask App)"]
```

Every browser request — whether it is loading the application, calling a REST API endpoint, or completing the Cognito Hosted UI login redirect — passes through CloudFront first. CloudFront redirects any HTTP request to HTTPS, then the CloudFront Function (`cognito-authorizer-cookie-to-header`) converts the Cognito access-token cookie into an `Authorization: Bearer <token>` header — it performs token forwarding only and does not validate the JWT. The request is then forwarded (via `AllViewerExceptHostHeader`) to the API Gateway origin at path `/saas`, where the Cognito User Pool Authorizer performs the actual JWT and scope validation, and the request continues the chain to the ALB and ultimately the ECS-hosted Flask application.

## 🔗 Communication With Other AWS Services

| Service | Interaction |
|---|---|
| Amazon API Gateway | Configured as the sole origin; the CloudFront Function (`cognito-authorizer-cookie-to-header`) converts the Cognito access-token cookie into an `Authorization: Bearer` header before CloudFront forwards all viewer requests to `26qdafcfw9.execute-api.us-east-1.amazonaws.com/saas` |
