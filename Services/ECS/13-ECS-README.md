...
sequenceDiagram
    ...
    ALB->>ECS: Forward HTTP request (Target Group saas-TG-12) — already JWT/scope-authorized by API Gateway
    ECS->>SM: Retrieve DB credentials (via tenant-saas-task-role)
    ECS->>ECS: Application-level tenant, role, resource, and business authorization
    ECS->>RDS: Query/write application data
    ECS->>SQS: SendMessage (tenant usage event)
    ECS-->>ALB: HTTP Response
```

Amazon ECS Fargate is the **application compute layer** — every request reaching this tier has already passed the Amazon API Gateway Cognito User Pool Authorizer, which provides the primary JWT authentication and API-scope authorization boundary. The Flask application performs application-level tenant, role, resource, and business authorization after the request passes that boundary; it does not independently validate the Cognito JWT as the primary authentication mechanism.
...
| **Amazon Cognito** | Flask exchanges the OAuth authorization code for tokens during the Hosted UI login callback (`/auth/callback`); it does not perform the primary JWT validation for protected API requests — that is enforced upstream by the API Gateway Cognito User Pool Authorizer before requests reach ECS |
...
- **Primary JWT authentication and API-scope authorization**: Amazon API Gateway with the Cognito User Pool Authorizer (`cognito-authorizer-06`) provides this boundary before requests reach the ECS Flask application.
- **Application-level authorization**: The Flask application performs application-level tenant, role, resource, and business authorization after the request passes the API Gateway authorization boundary. It is not the JWT authentication boundary itself.
...
Amazon ECS Fargate is the **core application runtime** of the entire platform. It hosts the Flask application that handles the Cognito Hosted UI login/callback flow, serves tenant dashboards, manages admin operations, and publishes billing events — making it the central compute component that every other service (ALB, Cognito, RDS, Secrets Manager, SQS) exists to support. Primary JWT authentication and API-scope authorization for protected routes is enforced upstream by the API Gateway Cognito User Pool Authorizer, before requests ever reach this tier.
