...
|---|---|
| Name | `saas-ALB-12` |
| ARN | `arn:aws:elasticloadbalancing:<AWS_REGION>:<AWS_ACCOUNT_ID>:loadbalancer/app/saas-ALB-12/<HASH>` |
...
| Attribute | Value |
|---|---|
| ARN | `arn:aws:iam::<AWS_ACCOUNT_ID>:role/tenant-saas-task-role` |
...
### 5.2 `tenant-saas-metering-role-<SUFFIX>` (Lambda Execution Role)

| Attribute | Value |
|---|---|
| ARN | `arn:aws:iam::<AWS_ACCOUNT_ID>:role/service-role/tenant-saas-metering-role-<SUFFIX>` |
| Trusted Entity | `lambda.amazonaws.com` |
| Policies | `AWSLambdaVPCAccessExecutionRole` (managed), `AWSLambdaBasicExecutionRole-<SUFFIX>` (customer managed) |
| Inline Policies | `secrets-lambda-policy` (`secretsmanager:GetSecretValue` on `<SECRET_ARN>`; `kms:Decrypt` on `<KMS_KEY_ARN>`), `sqs-lambda-policy` (`DeleteMessage`, `ReceiveMessage`, `GetQueueAttributes` on `<SQS_QUEUE_ARN>`) |
...
| Attribute | Value |
|---|---|
| ARN | `arn:aws:iam::<AWS_ACCOUNT_ID>:role/ecsTaskExecutionRole` |
| Trusted Entity | `ecs-tasks.amazonaws.com` |
...
| Execution Role | `tenant-saas-metering-role-<SUFFIX>` |
...
| Repository URI | `<AWS_ACCOUNT_ID>.dkr.ecr.<AWS_REGION>.amazonaws.com/saas` |
| Type | Private |
...
|---|---|
| `<AWS_ACCOUNT_ID>` | AWS account number in all ARNs |
...
| `<SECRET_NAME>`, `<SECRET_ARN>` | Secrets Manager identifiers |
...
| `<COGNITO_USER_POOL_ID>`, `<CLIENT_ID>`, `<COGNITO_DOMAIN>` | Cognito identifiers |
